...
public class AddReleaseRefTest
{
    ...
    private void AddReleaseRefTests()
    {
        try //Test for IntPtr.Zero
        {
            ...
            Assert.ErrorWriteLine("No exception from AddRef when passed IntPtr.Zero as parameter.");
        }
        ...
        {
            Assert.ErrorWriteLine("Unexpected Exception occurred from AddRef when passed IntPtr.Zero as parameter: {0}", ex);
        }

        try //Test for IntPtr.Zero
        {
            Marshal.Release(IntPtr.Zero);
            Assert.ErrorWriteLine("No exception from Release when passed IntPtr.Zero as parameter.");
        }
        catch (ArgumentNullException)
        {
        ...
        {
            Assert.ErrorWriteLine("Unexpected Exception occurred from Release when passed IntPtr.Zero as parameter: {0}", ex);
        }
        ...
        foreach (object obj in TestObjects)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < 10; i++)  //By the end of this loop we should have no additional refCount
                {
                    ...
                    {
                        Assert.ErrorWriteLine("Unexpected ref count. Expected: " + refCount + ", Actual: " + retValue);
                    }
            ...
            {
                Assert.ErrorWriteLine("Unexpected Exception occurred from GetIUnknownForObject: {0}", ex);
            }
        }
    }
